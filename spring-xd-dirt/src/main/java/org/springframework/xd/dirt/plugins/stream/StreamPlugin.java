...
public class StreamPlugin implements Plugin {
	...
	private void registerConsumer(Module module, MessageBus bus) {
		...
		if (channel != null) {
			if (isChannelPubSub(md.getInputChannelName())) {
				bus.registerPubSubConsumer(md.getInputChannelName(), channel, getAcceptedMediaTypes(module));
			}
			else {
				bus.registerConsumer(md.getInputChannelName(), channel, getAcceptedMediaTypes(module),
						md.isAliasedInput());
			}
		}
	}
	...
	private void registerProducer(Module module, MessageBus bus) {
		...
		if (channel != null) {
			if (isChannelPubSub(md.getOutputChannelName())) {
				bus.registerPubSubProducer(md.getOutputChannelName(), channel);
			}
			else {
				bus.registerProducer(md.getOutputChannelName(), channel, md.isAliasedOutput());
			}
			...
			if (channel instanceof AbstractMessageChannel) {
				...
				bus.registerPubSubProducer(tapChannelName, tapChannel);
			}
		}
	}
	...
	private void unregisterConsumer(Module module, MessageBus bus) {
		...
		if (inputChannel != null) {
			bus.unregisterConsumer(module.getDeploymentMetadata().getInputChannelName(), inputChannel);
		}
	}
	...
	private void unregisterProducer(Module module, MessageBus bus) {
		...
		if (outputChannel != null) {
			bus.unregisterProducer(module.getDeploymentMetadata().getOutputChannelName(), outputChannel);
		}
		bus.unregisterProducers(getTapChannelName(module));
	}
