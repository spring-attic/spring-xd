...
public class BatchStepExecutionsControllerIntegrationTests extends AbstractControllerIntegrationTest {
	...
	public void before() throws Exception {
		...
		final JobExecution jobExecution1 = new JobExecution(jobInstance1, 0l, jobParameters1);
		...
		when(jobService.getStepExecutions(5555L)).thenThrow(
				new NoSuchJobExecutionException("No JobExecution with id=555L"));
	...
	public void testGetBatchStepExecutions() throws Exception {
		mockMvc.perform(
				...
				.andExpect(jsonPath("$[*].jobExecutionId", contains(0, 0, 0)))
				.andExpect(jsonPath("$[*].stepExecution.stepName", contains("step1", "step2", "step3")))
				.andExpect(jsonPath("$[*].links[*].href", contains(
						"http://localhost/batch/executions/0/steps/1",
						"http://localhost/batch/executions/0/steps/2",
						"http://localhost/batch/executions/0/steps/3")));
	}
