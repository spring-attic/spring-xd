...
public class StreamPluginTests {
	...
	public void streamChannelTests() {
		...
		plugin.preProcessModule(module);
		plugin.postProcessModule(module);
		verify(bus).registerConsumer("foo.0", input, Collections.singletonList(MediaType.ALL), false);
		verify(bus).registerProducer("foo.1", output, false);
		verify(bus).registerPubSubProducer(eq("tap:foo.testing"), any(DirectChannel.class));
		plugin.beforeShutdown(module);
		plugin.removeModule(module);
		verify(bus).unregisterConsumer("foo.0", input);
		verify(bus).unregisterProducer("foo.1", output);
		verify(bus).unregisterProducers("tap:foo.testing");
	}
