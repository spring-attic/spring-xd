...
/**
 * Transformer implementation that converts a JSON string payload into an instance of the provided target Class. By
 * default this transformer uses {@linkplain JacksonJsonObjectMapperProvider} factory to get an instance of Jackson 1 or
 * Jackson 2 JSON-processor {@linkplain JsonObjectMapper} implementation depending on the jackson-databind or
 * jackson-mapper-asl libs on the classpath. Any other {@linkplain JsonObjectMapper} implementation can be provided.
 * 
 * @author Mark Fisher
 * @author Artem Bilan
...
public class JsonToObjectTransformer<T> extends AbstractPayloadTransformer<String, T> {
	...
	public JsonToObjectTransformer(Class<T> targetClass, JsonObjectMapper<?> jsonObjectMapper) {
		...
		this.jsonObjectMapper = (jsonObjectMapper != null) ? jsonObjectMapper
