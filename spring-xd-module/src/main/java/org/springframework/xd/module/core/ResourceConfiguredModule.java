...
/**
 * A {@link org.springframework.xd.module.core.SimpleModule} configured using a bean definition resource (XML or Groovy)
 *
...
public class ResourceConfiguredModule extends SimpleModule {
	...
	public ResourceConfiguredModule(ModuleDescriptor descriptor, ModuleDeploymentProperties deploymentProperties, ClassLoader classLoader, ModuleOptions moduleOptions) {
	...
	protected void configureModuleApplicationContext(ModuleDefinition moduleDefinition ) {
		...
		if (resource != null && resource.exists() && resource.isReadable() && (resource.getFilename().endsWith(".xml") || resource.getFilename().endsWith(".groovy"))) {
