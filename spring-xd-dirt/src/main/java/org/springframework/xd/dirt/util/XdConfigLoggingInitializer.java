...
public class XdConfigLoggingInitializer implements ApplicationListener<ContextRefreshedEvent>, EnvironmentAware {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		if (isContainer) {
			...
			logHadoopDistro();
			// TODO: IG: Once AdminServer has zkClientConnect support, we can move this out of isContainer check.
			logZkConnectString();
		}
	...
	private void logHadoopDistro() {
		...
		logger.info("Hadoop distro config (--hadoopDistro) used: " + hadoopDistro);
		...
		if (!hdpVersionFromCP.contains(hadoopVersionsMap.get(HadoopDistro.valueOf(hadoopDistro)))) {
			for (HadoopDistro key : hadoopVersionsMap.keySet()) {
				if (hdpVersionFromCP.contains(hadoopVersionsMap.get(key))) {
					logger.error("Hadoop version detected from classpath : " + hdpVersionFromCP
							+ " but you provided --hadoopDistro " + hadoopDistro + ". Did you mean? --hadoopDistro "
							+ key.toString());
	...
	private void logZkConnectString() {
		...
		logger.info("Zookeeper Client-Connect String(s): " + zkConnectString);
	}
}
