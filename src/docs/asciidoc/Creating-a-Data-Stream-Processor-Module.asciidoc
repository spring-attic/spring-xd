...
To implement a Stream based processor module you need to implement the interface `org.springframework.xd.reactor.ReactiveProcessor`
...
@EnableReactorModule
public class MovingAverage implements ReactiveProcessor<Tuple, Tuple> {

    @Override
    public void accept(Stream<Tuple> inputStream, ReactiveOutput<Tuple> output) {
        output.writeOutput(
          inputStream
...
You can also define a source or a sink if you implement the adequate interface:
...
@EnableReactorModule
public class PeriodicEmitter implements ReactiveSource<Long> {

    @Override
    public void accept(ReactiveOutput<Long> output) {
        output.writeOutput(
          Streams..period(1)
				);
    }
...
You can now create unit tests for the module just as you would for any other Java class.  The module application context file can be in XML or in Java using a @Configuration class.  The XML version is shown below.
