...
However, if you are interested in developing your own modules, some knowledge of Spring, Spring Integration or Spring Batch is essential. The remainder of this document assumes the reader has some familiarity with these topics.
...
==== Creating a Module Project
Spring XD (1.1.x or later) provides tools for creating a module project to test and package the module either with Maven or Gradle.
...
Note that Maven requires the parent pom version to be hard coded. 
...
[NOTE]
====
If your module has no additional dependencies, a plain old jar file conforming to the module layout shown above will work. In this case, you may still benefit from using these build tools to inherit common dependencies and automate tasks needed for in-container testing.
====

These build tools provide the necessary Spring XD libraries to compile and test the module along with support for packaging your module as an uber jar, wrapping the respective Spring Boot plugin: http://docs.spring.io/spring-boot/docs/current/reference/html/build-tool-plugins-maven-plugin.html[Spring Boot Maven Plugin] or the http://docs.spring.io/spring-boot/docs/current/reference/html/build-tool-plugins-gradle-plugin.html[Spring Boot Gradle Plugin].

As described in the above sections, the module jar must include any dependencies that are not provided by the Spring XD container. These are loaded at runtime by the module class loader when the module is deployed. Missing jars in the module's lib directory will result in the dreaded `ClassDefNotFoundException`. Conversely, the module should avoid exporting different versions of libraries which are already on the Spring XD class path as this can result in version conflicts and related class loading issues. Both build support tools configure the boot plugin with the `MODULE` layout and each is configured to exclude any artifacts that are already provided by Spring XD (which covers quite a lot). There are two basic rules:

* The `MODULE` layout for Spring Boot packaging ensures `provided` dependencies will not be included in the uber jar. The Spring XD module tools declare `spring-xd-dirt` as a provided dependency, as some of its classes are needed for module development.
* Any compile or runtime dependencies, transitive or explicitly declared, for the module will be excluded from the uber jar if they are also Spring XD runtime dependencies.
...
As of Spring XD 1.2.0, the Maven `spring-xd-module-parent` pom includes a `dependencyManagement` section, so you need not specify a version for shared Spring XD dependencies when using Maven. Also, the Maven build requires some extra configuration to override versions of existing Spring XD dependencies: 
...
Gradle requires you to specify a version, and will use the version you specify. However if you declare the dependency as `provided` Gradle will not export the dependency regardless of the version. 

===== Porting to another Spring XD version

As we have seen, a module project's build script is configured for a specific Spring XD version. With each new release of Spring XD, its runtime dependencies are subject to change and this directly affects which dependencies will be exported to the module jar and may result in version conflicts or unsatisfied dependencies. For this reason, _**we highly recommend that you rebuild any custom modules to match your target runtime environment.**_  In many cases, this is simply a matter of updating the spring XD version in your build. For example, if using Gradle, as discussed above, we recommend defining the required  `springXdVersion` property in `gradle.properties`. Then you may override this value on the command line without making any changes to the build script. For example, 

----
$./gradlew clean build -PspringXdVersion=1.2.0.BUILD-SNAPSHOT
----

[NOTE]
====
Overriding the property on the Gradle command line does not work if the property is hard coded in `build.gradle` itself, e.g., in an `ext` closure.
====

[NOTE]
====
If using Maven, the module build script must be modified to change the parent pom version, since Maven does not support the use of properties in this case.
====

In rare cases, other version changes may be required. For example, your module may declare compile or runtime dependencies that have since been added to the target Spring XD release. 
