...
Files will be replicated on the local filesystem in a temporary directory, on demand and loaded from there. Obviously, the XD Admin process will need to have write access to that shared HDFS directory. Intermediary paths (`/root/path/of/registry` in the example above) are created at startup if they don't exist yet.
