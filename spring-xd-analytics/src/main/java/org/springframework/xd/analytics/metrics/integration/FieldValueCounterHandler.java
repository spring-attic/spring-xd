...
public class FieldValueCounterHandler {
	...
	private Map<String, String> fieldNameToCounterNameMap;
	
	
	public FieldValueCounterHandler(FieldValueCounterService fieldValueCounterService, String counterName, String fieldName) {
		...
		fieldNameToCounterNameMap = new HashMap<String,String>();
	...
	protected void processValue(String counterName, Object value) {
		if ((value instanceof Collection) || ObjectUtils.isArray(value)) {			
			Collection<?> c = (value instanceof Collection) ?
					(Collection<?>) value : Arrays.asList(ObjectUtils.toObjectArray(value));
			for (Object val : c) {
