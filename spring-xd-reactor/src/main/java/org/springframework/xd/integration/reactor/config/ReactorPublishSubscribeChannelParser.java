...
public class ReactorPublishSubscribeChannelParser extends AbstractChannelParser {
	...
	protected BeanDefinitionBuilder buildBeanDefinition(Element element, ParserContext parserContext) {
		...
		String dispatcherBeanName = element.getAttribute("dispatcher");
		if (!StringUtils.hasText(dispatcherBeanName)) {
			builder.addConstructorArgValue(new ReactorProcessorMessageDispatcher());
		} else {
			builder.addConstructorArgReference(dispatcherBeanName);
		}
