 ...
 */
public class KMeansClusteringPmmlAnalyticTest extends AbstractPmmlAnalyticTest {

	/**
	 * @see XD-1420
	 */
	@Test
	public void testKmeansClusteringIris1() {

		Analytic<Tuple, Tuple> analytic = useAnalytic("iris-kmeans-clustering-1", null, Arrays.asList("predictedValue"));

		Tuple output = analytic.evaluate(objectToTuple(new Object() {

			@Value("Sepal.Length")
			double sepalLength = 6.4;

			@Value("Sepal.Width")
			double sepalWidth = 3.2;

			@Value("Petal.Length")
			double petalLength = 4.5;

			@Value("Petal.Width")
			double petalWidth = 1.5;
		}));
		...
		output = analytic.evaluate(objectToTuple(new Object() {
			...
			@Value("Sepal.Length")
			double sepalLength = 6.9;

			@Value("Sepal.Width")
			double sepalWidth = 3.1;

			@Value("Petal.Length")
			double petalLength = 5.4;

			@Value("Petal.Width")
			double petalWidth = 2.1;
		}));
