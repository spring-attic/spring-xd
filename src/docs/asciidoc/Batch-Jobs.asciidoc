...
----
xd:> job create incrementalImportJob --definition "jdbchdfs --columns='id,col1,col2' --tableName=some_table --checkColumn=sequence" --deploy
----
...
* If an import fails, it must be rerun to completion before running the next import.  Without this, inconsistent data may result.
