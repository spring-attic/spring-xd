...
public class TriggerModulesTest extends AbstractStreamIntegrationTest {
	...
	public void receivesContentsInstantlyByDefault() {
		stream().create(getRandomStreamName(), "trigger --payload='Hello' | %s", binaryFileSink);
	...
	public void doesNotReceiveAnyContentsForAFarFuture() {
		stream().create(
				getRandomStreamName(), "trigger --payload='Hello' --date='25/12/2032' --dateFormat='dd/MM/yyyy' | %s",
				binaryFileSink
				);
