...
/**
 * Samples throughput by counting messages over an elapsed time between receiving a {@link
 * org.springframework.messaging.Message} with the given {@code startMessage} and a {@code endMessage} and reports the
 * throughput in the given {@link java.util.concurrent.TimeUnit}.
 *
 * @author Jon Brisbin
 */
public class ThroughputSamplerMessageHandler implements MessageHandler {
  ...
  /**
   * Create a {@literal ThroughputSamplerMessageHandler} using the {@code String} {@literal START} as the start message
   * and the {@code String} {@literal END} as the end message and measure throughput in seconds.
   */
