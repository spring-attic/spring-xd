...
public class SyslogTest extends AbstractIntegrationTest {
	...
	public void testSyslogTcp() {
		...
		SyslogTcpSource syslogTcp = sources.syslogTcpSource().host(this.getContainerForStream().getHost());
	...
	public void testSyslogUdp() {
		String data = UUID.randomUUID().toString();
		SyslogUdpSource syslogUdp = sources.syslogUdpSource().host(this.getContainerForStream().getHost());
