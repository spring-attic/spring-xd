...
public class ReactorPeerFactoryBean<IN, OUT> implements FactoryBean<ReactorPeer<IN, OUT, ?>> {
	...
	public ReactorPeerFactoryBean(Environment env) {
		this(env, "tcp", null);
	}

	public ReactorPeerFactoryBean(Environment env, String transport) {
		this(env, transport, null);
	}

	public ReactorPeerFactoryBean(Environment env, Map<String, Codec> codecs) {
		this(env, "tcp", codecs);
	}
	...
	public ReactorPeerFactoryBean<IN, OUT> configure(URI uri) {
	...
	public ReactorPeer<IN, OUT, ?> getObject() throws Exception {
		...
		if (TCP.equals(transport)) {
			...
			return NetStreams.httpServer(NettyHttpServer.class, new NetStreams.HttpServerFactory<IN, OUT>() {
				@Override
				public Spec.HttpServerSpec<IN, OUT> apply(Spec.HttpServerSpec<IN, OUT> spec) {
					commonSpecProperties(spec);
					//Add specifics to HTTP
					return spec;
				}
			});
		} else {
