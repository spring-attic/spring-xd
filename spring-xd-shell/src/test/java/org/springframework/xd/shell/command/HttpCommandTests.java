...
public class HttpCommandTests extends AbstractStreamIntegrationTest {
	...
	/**
	 * This test will create a stream using a HTTP source and a File Sink.
	 * Subsequently the "http post" shell command is used to post a simple Ascii
	 * String to the admin server.
	 */
	@Test
	public void testHttpPostAsciiText() throws InterruptedException, IOException {
		...
		final File tempFile = testFolder.newFile("asciidata.txt");
		...
		final String stream = String.format("http --port=%s | file --name=%s --dir=%s", openPort, tempFileName, tempFileDir);

		logger.info("Creating Stream: " + stream);
		stream().create(streamName, stream);

		logger.info("Posting String: " + stringToPost);
		getShell().executeCommand(String.format("http post --target http://localhost:%s --data \"%s\"", openPort, stringToPost));
	...
	/**
	 * This test will create a stream using a HTTP source and a File Sink.
	 * Subsequently the "http post" shell command is used to post a UTF String
	 * (Japanese) to the admin server.
	 */
	@Test
	public void testHttpPostUtfText() throws InterruptedException, IOException {
		...
		final File tempFile = testFolder.newFile("utfdata.txt");
		final String tempFileName = tempFile.getName();
		final String tempFileDir = tempFile.getParentFile().getAbsolutePath();
		...
		getShell().executeCommand(String.format("http post --target http://localhost:%s --data \"%s\"", openPort, stringToPostInJapanese));

		final String resultData = FileUtils.readFileToString(tempFile, "UTF-8");
		assertEquals(StringEscapeUtils.unescapeJava(stringToPostInJapanese) + "\n", resultData);
	}
}
